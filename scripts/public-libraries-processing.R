...
#hist_lib_df_2016 <- hist_lib_df[hist_lib_df$`Fiscal Year` == "2016",]
...
income_all_years[income_all_years$Town.Library == "Ridgefield (no data provided)"] <-"Ridgefield"
...
#remove rows where Town.Library = LIBRARIES REPORTING
...
#merge in xwalk file
lib_town_xwalk_for_income <- read.csv(paste0(path_to_raw_data, "/", "library_town_crosswalk_for_income.csv"), stringsAsFactors = F, header = T)
...
# 
# #find all rows where rows have no NAs in initial df
# complete_df <- lib_df_agg[!rowSums(is.na(lib_df_agg)) > 0,]
# 
# #bring Town to first column
# complete_df <- complete_df[c(x, setdiff(names(complete_df), x))]
# 
# #sum complete_df
# complete_df_sum <- complete_df %>% 
#   group_by(Town, Fiscal.Year) %>% 
#   summarise(Total.Library.Visits = sum(Total.Library.Visits), 
#             Total.Registered.Borrowers = sum(Total.Registered.Borrowers), #no 0s
#             Reference.Questions = sum(Reference.Questions),
#             Total.Circulation = sum(Total.Circulation),
#             Total.Programs = sum(Total.Programs),
#             Total.Program.Attendance = sum(Total.Program.Attendance),
#             Use.of.Public.Internet.Computers = sum(Use.of.Public.Internet.Computers), #no 0s
#             Total.Collection.Size = sum(Total.Collection.Size), #no 0s
#             Total.Operating.Income = sum(Total.Operating.Income),
#             Municipal.Appropriation = sum(Municipal.Appropriation),
#             Library.Materials.Expenditures = sum(Library.Materials.Expenditures), #no 0s
#             Wages...Salaries.Expenditures = sum(Wages...Salaries.Expenditures),
#             Operating.Expenditures = sum(Operating.Expenditures)) %>% 
#   complete(Town, Fiscal.Year)
# 
# #find all rows where rows do have NAs in initial df
# NA_df <- lib_df_agg[rowSums(is.na(lib_df_agg)) > 0,]
# 
# #bring Town to first column
# NA_df <- NA_df[c(x, setdiff(names(NA_df), x))]
# 
...
# 
# #separate multiple libraries from single libraries
# NA_df_sing <- NA_df[!NA_df$Town %in% multiple_lib,] #True NAs no duplicates
# test <- NA_df_sing[,c("Town", "Fiscal.Year")]
# test2<-test[duplicated(test), ]
# 
# NA_df_sing <- NA_df_sing[,-c(3,4)]
# #essentially blank
# NA_df_sing2 <- NA_df_sing[colSums(is.na(NA_df_sing[c(2:15)])) < 2L,]
# 
# 
# 
# 
...
# 
# 
# NA_df_mult <- NA_df[NA_df$Town %in% multiple_lib,] #Not true NAs, with duplicates
# #remove pop, and rank columns
# NA_df_mult <- NA_df_mult[,-c(3,4)]
# test <- NA_df_mult[,c("Town", "Fiscal.Year")]
# test2<-test[duplicated(test), ]
# 
# #sum NA_df_mult with na.rm=T
# NA_df_mult_sum <- NA_df_mult %>% 
#   group_by(Town, Fiscal.Year) %>% 
#   summarise(Total.Library.Visits = sum(Total.Library.Visits, na.rm=T), 
#             Total.Registered.Borrowers = sum(Total.Registered.Borrowers, na.rm=T), #no 0s
#             Reference.Questions = sum(Reference.Questions, na.rm=T),
#             Total.Circulation = sum(Total.Circulation, na.rm=T),
#             Total.Programs = sum(Total.Programs, na.rm=T),
#             Total.Program.Attendance = sum(Total.Program.Attendance, na.rm=T),
#             Use.of.Public.Internet.Computers = sum(Use.of.Public.Internet.Computers, na.rm=T), #no 0s
#             Total.Collection.Size = sum(Total.Collection.Size, na.rm=T), #no 0s
#             Total.Operating.Income = sum(Total.Operating.Income, na.rm=T),
#             Municipal.Appropriation = sum(Municipal.Appropriation, na.rm=T),
#             Library.Materials.Expenditures = sum(Library.Materials.Expenditures, na.rm=T), #no 0s
#             Wages...Salaries.Expenditures = sum(Wages...Salaries.Expenditures, na.rm=T),
#             Operating.Expenditures = sum(Operating.Expenditures, na.rm=T)) %>% 
#   complete(Town, Fiscal.Year)
# 
# 
# complete_df_sum <- as.data.frame(complete_df_sum)
# NA_df_mult_sum <- as.data.frame(NA_df_mult_sum)
# 
# #bind together all formats
# total_df_sum <- rbind(complete_df_sum, NA_df_sing, NA_df_mult_sum)
# 
# test <- total_df_sum[,c("Town", "Fiscal.Year")]
# test2<-test[duplicated(test), ]
# 
# 
# 
...
#Rows with complete (NAs are coerced to 0s for sum function)
# lib_df_sum <- lib_df_agg %>% 
#   group_by(Town, Fiscal.Year, AENGLC.Wealth.Rank) %>% 
#   summarise(Total.Library.Visits = sum(Total.Library.Visits, na.rm=T), 
#             Total.Registered.Borrowers = sum(Total.Registered.Borrowers, na.rm=T), #no 0s
#             Reference.Questions = sum(Reference.Questions, na.rm=T),
#             Total.Circulation = sum(Total.Circulation, na.rm=T),
...
#             Wages...Salaries.Expenditures = sum(Wages...Salaries.Expenditures, na.rm=T),
#             Operating.Expenditures = sum(Operating.Expenditures, na.rm=T)) %>% 
#   complete(Town, Fiscal.Year, AENGLC.Wealth.Rank)




# NA_df <- NA_df[NA_df$Town %in% mulitple_lib == NA,] <- 0
# 
# x <- NA_df$Town[NA_df$Town %in% multiple_lib]
# NA_df2 <- NA_df %>% mutate(x = replace(is.na(x), 0))
# NA_df2<- NA_df %>% mutate(x = "is.na<-"(x, x < 0))
# 
# NA_df2 <- NA_df[is.na(NA_df$Town %in% multiple_lib),] <- 0
# 
# 
# #isolate duplicate rows
# test <- NA_df[,c("Town", "Fiscal.Year")]
# test2<-test[duplicated(test), ]
# test3<-test[!duplicated(test),]
# #set any NAs in NA_df that match these pairs of town and year to 0
# 
# replace <- merge(NA_df, test2, all.y=T)
# keep3 <- merge(NA_df, test3, all.y=T) #contains the true NAs
# 
# #sum these rows
# replace_sum <- replace %>% 
#   group_by(Town, Fiscal.Year, AENGLC.Wealth.Rank, Population.of.Service.Area) %>% 
#   summarise(Total.Library.Visits = sum(Total.Library.Visits, na.rm=T), 
#             Total.Registered.Borrowers = sum(Total.Registered.Borrowers, na.rm=T), #no 0s
#             Reference.Questions = sum(Reference.Questions, na.rm=T),
#             Total.Circulation = sum(Total.Circulation, na.rm=T),
...
#             Wages...Salaries.Expenditures = sum(Wages...Salaries.Expenditures, na.rm=T),
#             Operating.Expenditures = sum(Operating.Expenditures, na.rm=T)) %>% 
#   complete(Town, Fiscal.Year, AENGLC.Wealth.Rank, Population.of.Service.Area)
# 
# replace_sum2 <- as.data.frame(replace_sum)
# 
# #bind together replace_sum and keep3
# complete_test <- rbind(replace_sum2, keep3) #only rows that have NAs, now they need to merge back in the rest of data
# 
# #Take these rows and replace these rows with those that match in lib_df_sum df
# #First take out all rows that we are replacing
# #this is what we want to keep 
# keep <- anti_join(lib_df_sum, NA_df, by=c("Fiscal.Year", "Town", "AENGLC.Wealth.Rank", "Population.of.Service.Area"))
# 
# keep2 <- as.data.frame(keep)
# #Get two data sets in a state where we can merge them
# complete <- rbind(keep2, complete_test)
# 
# #check for duplicates that are ok (towns with multiple libraries)
# 
# 
...
# #set all 0s to NAs
# wages$sum_wage_sal_exp[wages$sum_wage_sal_exp == 0] <- NA
# 
...
# #set all 0s to NAs
# visits$sum_visits[visits$sum_visits == 0] <- NA
# 
# 
...
#   filter (Fiscal.Year %in% years)
# #set all 0s to NAs
# reg_borrow$sum_borrowers[reg_borrow$sum_borrowers == 0] <- NA
# 
...
#                                                           lib_df_sum$Fiscal.Year == "2007" | lib_df_sum$Fiscal.Year == "2008"))),]
# #set all 0s to NAs
# ref_ques$sum_ref_ques[ref_ques$sum_ref_ques == 0] <- NA
# 
# no_web <- c("1996", "1997", "1998", "1999", "2000", "2001", "2002", "2003", "2004")
# internet_merge <- get(true_NAs[5])
# backfill_for_internet <- expand.grid(
#   `Fiscal.Year` = no_web,
#   `Town` = unique(lib_df_sum$Town)
# )
...
# #set all 0s in internet column to NAs
# internet$sum_internet[internet$sum_internet == 0] <- NA
# 
# total_NA_backfill <- rbind(reg_borrow, visits, wages, ref_ques, internet)
# 
# #check to see if any duplicates in backfill data set
# test <- total_NA_backfill[,c("Fiscal.Year", "Town")]
# test2<-test[duplicated(test), ]
